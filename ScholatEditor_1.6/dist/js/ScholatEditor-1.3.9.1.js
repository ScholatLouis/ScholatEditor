...
(function (factory) {
        if (typeof define === "function" && define.amd) {
            // AMD模式
            define('ScholatEditor', ["jquery"], factory);
        } else {
            // 全局模式
...
(function($){
    $(function(){
        ...
        if(isIE6 || isIE7 || isIE8){
            ...
            window.onload = function() {
                ...
                var icons = {
                    'icon-ScholatEditor-link' : '&#xe800;',
                    'icon-ScholatEditor-unlink' : '&#xe801;',
                    'icon-ScholatEditor-code' : '&#xe802;',
                    'icon-ScholatEditor-cancel': '&#xe803;',
                    'icon-ScholatEditor-terminal':'&#xe804;',
                    'icon-ScholatEditor-angle-down':'&#xe805;',
                    'icon-ScholatEditor-font':'&#xe806;',
                    'icon-ScholatEditor-bold':'&#xe807;',
                    'icon-ScholatEditor-italic':'&#xe808;',
                    'icon-ScholatEditor-header':'&#xe809;',
                    'icon-ScholatEditor-align-left':'&#xe80a;',
                    'icon-ScholatEditor-align-center':'&#xe80b;',
                    'icon-ScholatEditor-align-right':'&#xe80c;',
                    'icon-ScholatEditor-list-bullet':'&#xe80d;',
                    'icon-ScholatEditor-indent-left':'&#xe80e;',
                    'icon-ScholatEditor-indent-right':'&#xe80f;',
                    'icon-ScholatEditor-list-numbered':'&#xe810;',
                    'icon-ScholatEditor-underline':'&#xe811;',
                    'icon-ScholatEditor-table':'&#xe812;',
                    'icon-ScholatEditor-eraser':'&#xe813;',
                    'icon-ScholatEditor-text-height':'&#xe814;',
                    'icon-ScholatEditor-brush':'&#xe815;',
                    'icon-ScholatEditor-pencil':'&#xe816;',
                    'icon-ScholatEditor-minus':'&#xe817;',
                    'icon-ScholatEditor-picture':'&#xe818;',
                    'icon-ScholatEditor-file-image':'&#xe819;',
                    'icon-ScholatEditor-cw':'&#xe81a;',
                    'icon-ScholatEditor-ccw':'&#xe81b;',
                    'icon-ScholatEditor-music':'&#xe911;',
                    'icon-ScholatEditor-play':'&#xe912;',
                    'icon-ScholatEditor-location':'&#xe947;',
                    'icon-ScholatEditor-happy':'&#xe9df;',
                    'icon-ScholatEditor-sigma':'&#xea67',
                    'icon-ScholatEditor-enlarge2':'&#xe98b;',
                    'icon-ScholatEditor-shrink2':'&#xe98c;',
                    'icon-ScholatEditor-newspaper':'&#xe904;',
                    'icon-ScholatEditor-camera':'&#xe90f;',
                    'icon-ScholatEditor-video-camera':'&#xe914;',
                    'icon-ScholatEditor-file-zip':'&#xe92b;',
                    'icon-ScholatEditor-stack':'&#xe92e;',
                    'icon-ScholatEditor-credit-card':'&#xe93f;',
                    'icon-ScholatEditor-address-book':'&#xe944;',
                    'icon-ScholatEditor-envelop':'&#xe945;',
                    'icon-ScholatEditor-drawer':'&#xe95c;',
                    'icon-ScholatEditor-download':'&#xe960;',
                    'icon-ScholatEditor-upload':'&#xe961;',
                    'icon-ScholatEditor-lock':'&#xe98f;',
                    'icon-ScholatEditor-unlocked':'&#xe990;',
                    'icon-ScholatEditor-wrench':'&#xe991;',
                    'icon-ScholatEditor-eye':'&#xe9ce;',
                    'icon-ScholatEditor-eye-blocked':'&#xe9d1;',
                    'icon-ScholatEditor-command':'&#xea4e;',
                    'icon-ScholatEditor-font2':'&#xea5c;',
                    'icon-ScholatEditor-libreoffice':'&#xeade;',
                    'icon-ScholatEditor-quotes-left':'&#xe977;',
                    'icon-ScholatEditor-strikethrough':'&#xea65;',
                    'icon-ScholatEditor-directory':'&#xea66;'
                };

                //遍历菜单按钮，替换fontIcon
                $('.ScholatEditor-container i').each(function(){
                    var elem = this,
                        className = this.className,
                        matchs = className.match(/icon-SchcolatEditor-[^\s'"]+/);
                    if (matchs) {
    ...
    //检测jquery是否正常
    if(!$){
    	alert('检测到页面没有引用jQuery，请先引用，否则ScholatEditor将无法使用。');
        return;
    } else if(typeof $ !== 'function' || /^\d+\.\d+\.\d+$/.test($().jquery) === false){
    	alert('检测到 window.jQuery 已被修改，ScholatEditor无法使用。');
        return;
    }
    //------------------------------------定义全局变量------------------------------------
    var document = window.document,
        ...
        //id前缀
        idPrefix = 'ScholatEditor_' + Math.random().toString().replace('.', '') + '_',
        globalNum = 1,
    ...
    $.extend($E, {
        ...
        'consoleLog': function(info){
            if(window.console && window.console.log && typeof window.console.log === 'function'){
                window.console.log('ScholatEditor提示：', info);
            }
        },
        ...
        'showTableBorder': function($content){
            $content.find('table').each(function(){
                var $this = $(this),
                    mark = 'ScholatEditor_table_border_mark',
    ...
    $.extend($E, {
        ...
        'styleConfig': {
            ...
            'scholatHeadersStyle': 'font-size: 24px; font-weight: bold; line-height: 24px; border-bottom: 1px solid #dedfe1; padding-bottom: 5px; margin: 35px 0 15px; clear: both;'
        },

        //html模板，在前台添加的html代码
        'htmlTemplates': {
            //删除table,img的按钮
            'elemDeleteBtn': '<a href="#" class="ScholatEditor-elemDeleteBtn"><i class="icon-ScholatEditor-cancel"></i></a>',
            'imgResizeBtn': '<div class="ScholatEditor-imgResize"></div>',
            //整个编辑器的容器
            'editorContainer': '<div class="ScholatEditor-container"></div>',
            //菜单容器（加上clearfix）
            'btnContainer': '<div class="ScholatEditor-btn-container clearfix"></div>',
            //菜单组
            'btnContainerGroup': '<div class="ScholatEditor-btn-container-group"></div>',
            //单个菜单按钮（一定要有 herf='#'，否则无法监听blur事件）
            'btn': '<a class="ScholatEditor-btn-container-btn ScholatEditor-btn-container-btn-default" href="#"></a>', 
            //下拉按钮右侧的小三角
            'btnAngleDown': '<i class="icon-ScholatEditor-angle-down" style="margin-left:3px;"></i>',
            //btn tooltip
            'tooltip': '<div class="ScholatEditor-toolTip"></div>',
            //btn tooltipContent
            'tooltipContent': '<div class="ScholatEditor-toolTip-content">{title}</div>',
            //所有弹出框modal的容器
            'modalContainer': '<div class="ScholatEditor-modal-container"></div>',
            //modal（按大小分为4种）
            'modal': '<div class="ScholatEditor-modal">{content}</div>',
            'modalBig': '<div class="ScholatEditor-modal ScholatEditor-modal-big">{content}</div>',
            'modalSmall': '<div class="ScholatEditor-modal ScholatEditor-modal-small">{content}</div>',
            'modalMini': '<div class="ScholatEditor-modal ScholatEditor-modal-mini">{content}</div>',
            //modal header
            'modalHeader': '<div class="ScholatEditor-modal-header clearfix">' + 
                                '<a href="#" commandName="close" class="ScholatEditor-modal-header-close"><i class="icon-ScholatEditor-cancel"></i></a>' + 
                                '<b>{title}</b>' + 
                                '<div class="ScholatEditor-modal-header-line"></div>' + 
                            '</div>',
            //编辑框的容器
            'txtContainer': '<div class="ScholatEditor-textarea-container"></div>',
            //编辑框
            'txt': '<div class="ScholatEditor-textarea" contenteditable="true"><p><br/></p></div>',
            //dropmenu
            'dropMenu': '<ul class="ScholatEditor-drop-menu">{content}</ul>',
            //dropPanel
            'dropPanel': '<ul class="ScholatEditor-drop-panel">{content}</ul>',
            //dropPanel-big
            'dropPanelBig': '<ul class="ScholatEditor-drop-panel ScholatEditor-drop-panel-big">{content}</ul>',
            //dropPanel-floatItem
            'dropPanel_floatItem': '<div class="ScholatEditor-drop-panel-floatItem">{content}</div>',
            //视频
    ...
    $.extend($E, {
    	'getUploadImgComponentForCrossDomain': function(editor){
    		var uploadUrl = editor.uploadUrl,
    	        fileInputName = 'ScholatEditor_uploadImg',  //服务器端根据这个name获取file
    	    ...
    	    var content =   '<form id="' + formId + '" method="post" enctype="multipart/form-data" target="' + iframeId + '">'+
    	                    ...
    	                    '   <p><button id="' + btnId + '"  type="button" class="ScholatEditor-modal-btn">' +langUpload+ '</button></p>' +
    	    ...
    	    $uploadImg_modal.find('#' + btnId).click(function(e){
    	        ...
    	        //定义callback事件
    	        window.ScholatEditor_uploadImgCallback = function(result){
    	            //提示已经开始调用
    	            $E.consoleLog('父页面的ScholatEditor_uploadImgCallback方法已经开始被调用！');
    	            ...
    	            if(result.indexOf('ok|') === 0){
    	                ...
    	                //获取图片url
    	                url = arr[1];
    	                $E.consoleLog('ScholatEditor_uploadImgCallback方法成功获取到图片url：' + url);
    	                ...
    	                if(arr.length === 2){
    	                    ...
    	                    //有缩略图形式
    	                    thumbnailUrl = arr[2];
    	                    $E.consoleLog('ScholatEditor_uploadImgCallback方法成功获取到缩略图url：' + thumbnailUrl);
    	                ...
    	                //提示成功插入图片
    	                $E.consoleLog('ScholatEditor_uploadImgCallback方法已经成功插入图片，弹出框也被关闭！');
    	            ...
    	            //用完立刻清除，防止影响其他editor
    	            window.ScholatEditor_uploadImgCallback = undefined;
    	        };
    ...
    $.extend($E, {
        ...
        //创建一个菜单组
        'createMenuGroup': function(menuGroup, editor, options){
            ...
            $.each(menuGroup, function(key, menuId){
                //遍历这个菜单组，创建每一个菜单
                var btn = $E.createMenu(menuId, editor, options);
        ...
        //创建一个菜单
        'createMenu': function(menu, editor, options){
            //验证
            if(menu == null){
                return;
            }
            ...
            if(style){
                if(options.lang === 'ch'){
                    if(cssClass === 'icon-ScholatEditor-upload'){
                        txt = '<i class="' + cssClass + '" style="' + style + '"> 保存内容</i>';        
                    } else if(cssClass === 'icon-ScholatEditor-cancel'){
                        txt = '<i class="' + cssClass + '" style="' + style + '"> 退出编辑</i>';
                    }else if(cssClass === 'icon-ScholatEditor-eraser'){
                        txt = '<i class="' + cssClass + '" style="' + style + '"> 清除格式</i>';
                    } else {
                        txt = '<i class="' + cssClass + '" style="' + style + '"></i>';    
                    }
                } else {
                    if(cssClass === 'icon-ScholatEditor-upload'){
                        txt = '<i class="' + cssClass + '" style="' + style + '"> save</i>';        
                    } else if(cssClass === 'icon-ScholatEditor-cancel'){
                        txt = '<i class="' + cssClass + '" style="' + style + '"> quit</i>';
                    }else if(cssClass === 'icon-ScholatEditor-eraser'){
                        txt = '<i class="' + cssClass + '" style="' + style + '"> reset</i>';
                    } else {
                        txt = '<i class="' + cssClass + '" style="' + style + '"></i>';    
                    }
                }
    ...
    $.extend($E.fn, {
        ...
        'init': function($textarea, options){
            ...
            */
            ...
            var //options
                onchange = options.onchange,                        //此时的值为undefined
                menuConfig = options.menuConfig,                    //此时的值为前台jsp设置的菜单栏值
                expressions = options.expressions,                  //此时的值为undefined
                uploadImgComponent = options.uploadImgComponent,    //此时的值为undefined
                uploadUrl = options.uploadUrl,                      //此时的值为undefined
                lang = options.lang,                                //如果前台没有设置语言，则默认使用中文
            ...
            //设置id
            //此处id的值为前台jsp设置textarea的id属性值
            var id = $textarea.attr('id');
            if(!id){
                //当有多个textarea具有多个相同id的时候，则通过getUniqeId获取第一个id作为编辑器，其余的则还是普通的textarea
                id = $E.getUniqeId();
            }
            editor.id = id;

            //创建基础DOM实体对象，并组合
            //htmlTemplates定义了html代码
            ...
            //设置编辑器的宽度 
            editor.$editorContainer.attr('style','margin-left: 210px; width: 78%;');
            ...
            //高度注释掉，因为不要在编辑器中出现滚动条
            //设置高度（必须在dom渲染完之后才行）
            $(function(){
                //计算txtContainer的高度，必须等待页面加载完成才能计算，否则dom没有被渲染，无法计算高度
                //editor.$btnContainer.outerHeight()获取的是菜单栏的高度
            ...
            $.each(editor.editorMenuConfig, function(key, menuGroup){
                $E.createMenuGroup(menuGroup, editor, options);
            });
            ...
            function txtListener(e){
                ...
                $E.isPassContent = false;   //声明一个全局变量用于记录，防止一个工程同时经过设置正文和设置目录
                //实现对标题进行实时监听
                var $appendHeader = new Array();

                //提醒scholatEditorTip
                var scholatTip = "";
                var scholatHeader = "";

                $temp = editor.$txt.find('h1.scholatHeader');
                $temp.each(function(index, value){
                    //当删除目录为空的时候，自动删除样式，并且对侧边栏的目录展示进行更新
                    if( $.trim($(value).text()) == "" ){
                        $(value).remove();
                    }
                    scholatHeader = $(value).text();
                    //设置中文状态下，目录长度不能超过10个字节
                    //设置英文状态下，目录长度不能超过30个字节
                    if(options.lang === 'ch'){
                        if( $.trim($(value).text()).length > 10 ){
                            editor.scholatEditorTip("scholatEditorTip-catalogOverBit", editor);
                            scholatHeader = $.trim($(value).text()).substring(0,10);
                            $(value).text(scholatHeader);
                        }
                    } else {
                        if( $.trim($(value).text()).length > 30 ){
                            editor.scholatEditorTip("scholatEditorEnTip-catalogOverBit", editor);
                            scholatHeader = $.trim($(value).text()).substring(0,30);
                            $(value).text(scholatHeader);
                        }                       
                    }
           
                    $appendHeader.push("<p class='icon-ScholatEditor-command'><a style='text-decoration: none; color: #36c;' class='leftScholatHeaders' href='#" + scholatHeader + "'>" + scholatHeader + "</a></p>");
                });
                //每次添加目录的时候都将#showScholatHeader区域内容清空
                $("#showScholatHeader").html("");
                //将内容追加到#showScholatHeader区域中
                $("#showScholatHeader").append($appendHeader.join('\n'));

                if(e && e.type === 'focus'){
                    //focus只能执行一次监听——页面一加载时$txt被强制执行focus()，而剩下的监听都会由click和keyup代替
                    editor.$txt.off('focus', txtListener);
                }
                //时刻监听生成重新生成目录定位(不知道为什么需要？)
                //使用js平滑定位
                $('.leftScholatHeaders').click(function(event){
                    var href = $( this ).attr( 'href' );
                    var position = $( href ).offset().top - 30;
                    $('html,body').animate({"scrollTop": position}, 1000);
                    event.preventDefault();    //阻止href锚点起作用
                    // return false;
                });
            }
            ...
            .on('mousedown', function(){
                ...
                editor.command(eve, 'insertHTML', data);
                eve.preventDefault();   //阻止粘贴事件的发生
                // return false; 
            });
            ...
            //加载完编辑器，获取编辑器中的内容
            //如果没有内容则不执行
            if(editor.$txt.html() != null){
                var $preScholatHeader = editor.$txt.find("h1.scholatHeader");
                console.log($preScholatHeader.length);
                var $preAppendHeader = new Array();
                //生成目录
                $.each($preScholatHeader, function(idex, value){
                    $quote = $(value);
                    $preAppendHeader.push("<p class='icon-ScholatEditor-command'><a style='text-decoration: none; color: #36c;' class='leftScholatHeaders' href='#"+ $quote.text() + "'>" + $quote.text() + "</a></p>");
                });
                $("#showScholatHeader").html($preAppendHeader.join("\n"));
            }
            //使用js平滑定位
            $('.leftScholatHeaders').click(function(){
                console.log("halo");
                var href = $( this ).attr( 'href' );
                var position = $( href ).offset().top - 50;
                $('html,body').animate({"scrollTop": position}, 1000);
                return false;   //阻止href锚点起作用
            });
    ...
    $.extend($E.fn, {
        ...
        'insertScholatEditorModal': function(editor, options){
            var scrollTop = 0;
            var width = $(".ScholatEditor-container").width()/2 - 200;
            var tipContent = "";
            
            //插入中文提示Modal
            if(options.lang === 'ch'){
                tipContent = "<p id='{scholatEditorTipId}' class='scholatEditor-Header-Tip' style='display: none; z-index: 100001; margin-top:" + scrollTop + "px; margin-left: " + width + "px;'>{scholatTip}</p>";
                tipContent = tipContent.replace('{scholatTip}', "目录设置不能超过10个");
                tipContent = tipContent.replace('{scholatEditorTipId}', "scholatEditorTip-catalogOver");
                $lilnk_modal = $(
                    $E.htmlTemplates.modalMini.replace( '{content}', tipContent )
                );
                editor.insertModal(tipContent);

                tipContent = "<p id='{scholatEditorTipId}' class='scholatEditor-Header-Tip' style='display: none; z-index: 100001; margin-top:" + scrollTop + "px; margin-left: " + width + "px;'>{scholatTip}</p>";
                tipContent = tipContent.replace('{scholatTip}', "目录不能超过十个字节");
                tipContent = tipContent.replace('{scholatEditorTipId}', "scholatEditorTip-catalogOverBit");
                $lilnk_modal = $(
                    $E.htmlTemplates.modalMini.replace( '{content}', tipContent )
                );
                editor.insertModal(tipContent);
                ...
                tipContent = "<p id='{scholatEditorTipId}' class='scholatEditor-Header-Tip' style='display: none; z-index: 100001; margin-top:" + scrollTop + "px; margin-left: " + width + "px;'>{scholatTip}</p>";
                tipContent = tipContent.replace('{scholatTip}', "请输入正确的网址");
                tipContent = tipContent.replace('{scholatEditorTipId}', "scholatEditorTip-errorWebSite");
                $lilnk_modal = $(
                    $E.htmlTemplates.modalMini.replace( '{content}', tipContent )
                );
                editor.insertModal(tipContent);
                ...
                tipContent = "<p id='{scholatEditorTipId}' class='scholatEditor-Header-Tip' style='display: none; z-index: 100001; margin-top:" + scrollTop + "px; margin-left: " + width + "px;'>{scholatTip}</p>";
                tipContent = tipContent.replace('{scholatTip}', "编辑的内容保存成功");
                tipContent = tipContent.replace('{scholatEditorTipId}', "scholatEditorTip-contentSave");
                $lilnk_modal = $(
                    $E.htmlTemplates.modalMini.replace( '{content}', tipContent )
                );
                editor.insertModal(tipContent);

                tipContent = "<p id='{scholatEditorTipId}' class='scholatEditor-Header-Tip' style='display: none; width: 260px; z-index: 100001; margin-top:" + scrollTop + "px; margin-left: " + width + "px;'>{scholatTip}</p>";
                tipContent = tipContent.replace('{scholatTip}', "出错啦，请稍后再尝试保存！");
                tipContent = tipContent.replace('{scholatEditorTipId}', "scholatEditorTip-contentFail");
                $lilnk_modal = $(
                    $E.htmlTemplates.modalMini.replace( '{content}', tipContent )
                );
                editor.insertModal(tipContent);
                ...
                //插入英文提示Modal
                tipContent = "<p id='{scholatEditorTipId}' class='scholatEditor-Header-EnTip' style='display: none; z-index: 100001; margin-top:" + scrollTop + "px; margin-left: " + width + "px;'>{scholatTip}</p>";
                tipContent = tipContent.replace('{scholatTip}', "Directory setting up to 10!");
                tipContent = tipContent.replace('{scholatEditorTipId}', "scholatEditorEnTip-catalogOver");
                $lilnk_modal = $(
                    $E.htmlTemplates.modalMini.replace( '{content}', tipContent )
                );
                editor.insertModal(tipContent);
                ...
                tipContent = "<p id='{scholatEditorTipId}' class='scholatEditor-Header-EnTip' style='display: none; z-index: 100001; margin-top:" + scrollTop + "px; margin-left: " + width + "px;'>{scholatTip}</p>";
                tipContent = tipContent.replace('{scholatTip}', "Directory description more than permission!");
                tipContent = tipContent.replace('{scholatEditorTipId}', "scholatEditorEnTip-catalogOverBit");
                $lilnk_modal = $(
                    $E.htmlTemplates.modalMini.replace( '{content}', tipContent )
                );
                editor.insertModal(tipContent);
                ...
                tipContent = "<p id='{scholatEditorTipId}' class='scholatEditor-Header-EnTip' style='display: none; z-index: 100001; margin-top:" + scrollTop + "px; margin-left: " + width + "px;'>{scholatTip}</p>";
                tipContent = tipContent.replace('{scholatTip}', "Please enter correct webSite!");
                tipContent = tipContent.replace('{scholatEditorTipId}', "scholatEditorEnTip-errorWebSite");
                $lilnk_modal = $(
                    $E.htmlTemplates.modalMini.replace( '{content}', tipContent )
                );
                editor.insertModal(tipContent);
                ...
                tipContent = "<p id='{scholatEditorTipId}' class='scholatEditor-Header-EnTip' style='display: none; z-index: 100001; margin-top:" + scrollTop + "px; margin-left: " + width + "px;'>{scholatTip}</p>";
                tipContent = tipContent.replace('{scholatTip}', "Save Successfully!");
                tipContent = tipContent.replace('{scholatEditorTipId}', "scholatEditorEnTip-contentSave");
                $lilnk_modal = $(
                    $E.htmlTemplates.modalMini.replace( '{content}', tipContent )
                );
                editor.insertModal(tipContent);

                tipContent = "<p id='{scholatEditorTipId}' class='scholatEditor-Header-EnTip' style='display: none; width: 260px; z-index: 100001; margin-top:" + scrollTop + "px; margin-left: " + width + "px;'>{scholatTip}</p>";
                tipContent = tipContent.replace('{scholatTip}', "Error, Please try again!");
                tipContent = tipContent.replace('{scholatEditorTipId}', "scholatEditorEnTip-contentFail");
                $lilnk_modal = $(
                    $E.htmlTemplates.modalMini.replace( '{content}', tipContent )
                );
                editor.insertModal(tipContent);
        ...
        //隐藏modal
        'hideModal': function(){
            this.$modalContainer.find('.ScholatEditor-modal:visible').hide();

            //经测试，safari浏览器菜单按钮检测不到blur事件
            //因此，只能强制隐藏dropmenu和droppanel
            this.$btnContainer.find('.ScholatEditor-drop-panel:visible').hide();
            this.$btnContainer.find('.ScholatEditor-drop-menu:visible').hide();
        },

        'showScholatHeaderTip': function(id){
            var scrollTop = document.documentElement.scrollTop || window.pageYOffset || document.body.scrollTop;
            $('#' + id).css("margin-top",scrollTop);
            $('#' + id).show();
        },
    ...
    $.extend($E.fn, {
    	...
    	'initLang': function(lang){
    		var editor = this,
    			langs = $E.langs,
    			defaultLang = 'ch',  //默认语言为中文 ch
    			langConfig = langs[defaultLang];  //获取Ch配置的详细信息
    		...
    		//赋值
    		editor.defaultLang = defaultLang; //ch
    		editor.lang = lang;               //ch
    		editor.langConfig = langConfig;   //object对象
    	}
    });
            ...
            */
            ...
            editor.menus = {
                'bold': {
                    ...
                    'cssClass':'icon-ScholatEditor-bold',
                    'command': 'bold',
                ...
                'underline': {
                    ...
                    'style': 'color: #399',
                    'hotKey': 'ctrl + u',
                    'cssClass':'icon-ScholatEditor-underline',
                    'command': 'underline '
                },
                'italic': {
                    ...
                    'style': 'color: #399',
                    'hotKey': 'ctrl + i',
                    'cssClass':'icon-ScholatEditor-italic',
                    'command': 'italic '
                },
                'removeFormat': {
                    ...
                    'type': 'btn',
                    'style': 'color: #399',
                    'cssClass':'icon-ScholatEditor-eraser',
                ...
                'unLink': {
                    ...
                    'type': 'btn',
                    'style': 'color: #399',
                    'cssClass':'icon-ScholatEditor-unlink',
                    'command': 'unLink ' 
                },
                'insertHr': {
                    ...
                    'type': 'btn',
                    'style': 'color: #399',
                    'cssClass':'icon-ScholatEditor-minus',
                ...
                'strikethrough':{
                    ...
                    'type': 'btn',
                    'style': 'color: #399',
                    'cssClass':'icon-ScholatEditor-strikethrough',
                ...
                'blockquote': {
                    ...
                    'type': 'btn',
                    'style': 'color: #399',
                    'cssClass':'icon-ScholatEditor-quotes-left',
                    'command': 'formatBlock',
                ...
                'justify': {
                    ...
                    'type': 'dropMenu',
                    'style': 'color: #399',
                    'cssClass':'icon-ScholatEditor-align-left',
                    'dropMenu': function(){
                        var arr = [],
                            ...
                            data = [
                                {
                                    ...
                                    'txt': '<i class="icon-ScholatEditor-align-left"> ' + langMenus.justify.left.title + '</i>'
                                },{
                                    //居中
                                    'commandName': 'JustifyCenter',
                                    'txt': '<i class="icon-ScholatEditor-align-center"> ' + langMenus.justify.center.title + '</i>'
                                },{
                                    //右对齐
                                    'commandName': 'JustifyRight',
                                    'txt': '<i class="icon-ScholatEditor-align-right"> ' + langMenus.justify.right.title + '</i>'
                                }
                            ];
                ...
                'list': {
                    ...
                    'type': 'dropMenu',
                    'style': 'color: #399',
                    'cssClass':'icon-ScholatEditor-list-bullet',
                    'dropMenu': function () {
                        var arr = [],
                            temp = '<li><a href="#" customCommandName="${command}">${txt}</a></li>',
                            $ul,
                            data = [
                                {
                                    ...
                                    'txt': '<i class="icon-ScholatEditor-list-bullet"> ' + langMenus.list.unOrdered.title + '</i>'
                                },{
                                    //有序列表
                                    'commandName': 'InsertOrderedList',
                                    'txt': '<i class="icon-ScholatEditor-list-numbered"> ' + langMenus.list.ordered.title + '</i>'
                                }
                            ];
                ...
                'fontFamily': {
                    ...
                    'type': 'dropMenu',
                    'cssClass': 'icon-ScholatEditor-font2',
                    'command': 'fontName ',
                    'style': 'color: #399', 
                    'dropMenu': function(){
                        var arr = [],
                ...
                'fontSize': {
                    ...
                    'type': 'dropMenu',
                    'cssClass': 'icon-ScholatEditor-text-height',
                    'command': 'fontSize',
                    'style': 'color: #399',
                    'dropMenu': function () {
                        var arr = [],
                ...
                'setHead': {
                    ...
                    'type': 'dropMenu', 
                    'cssClass':'icon-ScholatEditor-directory',
                    'command': 'formatBlock ',
                    'style': 'color: #399',
                    'dropMenu': function(){
                        var head = langMenus.setHead.title,
                            content = langMenus.setHead.content;
                        var liListStr =  '<li><a href="#" commandValue="<h1>"><h3>' + head + '</h3></a></li>' + 
                                    '<li><a href="#" commandValue="<p>">' + content + '</a></li>';
                        return $( $E.htmlTemplates.dropMenu.replace('{content}', liListStr) );
                    },
                    'callback': function(e){    //callback函数是在'command': fucntion()执行之后再执行
                        var event = e.originalEvent;
                        var $scholatHeaders = editor.$txt.find('h1.scholatHeader');          //已经存在的目录
                        var $scholatContents = editor.$txt.find('p');                        //执行完commond函数之后，则变成p标签
                        var $appendHeader = new Array();                                    //用于存放追加到前台的内容
                        var isScholatHeader;                                                //是否为目录的标志

                        //设置为正文
                        if($scholatContents && $scholatContents != null){
                            $.each($scholatContents, function(index, value){
                                var $quote = $(value);
                                //如果设置为正文，则直接覆盖其他样式，样式为空
                                $quote.attr('style', '');
                            });
                        };

                        //如果已有的目录已满，则不再允许设置目录
                        if($scholatHeaders.length < 10){
                            //设置为目录
                            //如果含有scholatHeader类则已经为目录，否则则为文本
                            isScholatHeader = $( editor.parentElemForCurrentRange() ).parent().hasClass('scholatHeader');
                            //isScholatHeader为true只有一种情况，就是已经是目录，然后有一次选择设置目录
                            if( isScholatHeader ){
                                //已经是目录，再一次设置目录则不给予处理
                                return ;
                            } else if( !($E.isPassContent) ){
                                //非目录设置为目录
                                //排除掉目录设置为正文的情况
                                var selectedContent = editor.currentRange();
                                //设置中文状态下，则目录长度不能超过10个字节
                                //设置英文状态下，则目录长度不能超过30个字节
                                if(options.lang === 'ch'){
                                    if( $.trim( selectedContent ).length <= 10 ){
                                        if( $.trim(selectedContent).length != 0 && selectedContent != null ){
                                            var insertContent = "<h1 style='font-size: 24px; font-weight: bold; line-height: 24px; border-bottom: 1px solid #dedfe1; padding-bottom: 5px; margin: 35px 0 15px; clear: both;' class='scholatHeader' id='" + selectedContent + "'>" + selectedContent + "</h1>";
                                            editor.command(event, 'insertHTML', insertContent);
                                        }
                                    } else {
                                        editor.scholatEditorTip("scholatEditorTip-catalogOverBit", editor);
                                    }
                                } else {
                                    if( $.trim( selectedContent ).length <= 30 ){
                                        if( $.trim(selectedContent).length != 0 && selectedContent != null ){
                                            var insertContent = "<h1 style='font-size: 24px; font-weight: bold; line-height: 24px; border-bottom: 1px solid #dedfe1; padding-bottom: 5px; margin: 35px 0 15px; clear: both;' class='scholatHeader' id='" + selectedContent + "'>" + selectedContent + "</h1>";
                                            editor.command(event, 'insertHTML', insertContent);
                                        }
                            ...
                            //每次设置目录 不需要全部重新生成目录 只需要添加最新目录即可，因为editor.$txt.find('h1:not(.scholatHeader)')
                            //每次添加目录的时候都将#showScholatHeader区域内容清空
                            // $("#showScholatHeader").html("");
                            //将内容追加到#showScholatHeader区域中
                            $("#showScholatHeader").append($appendHeader.join('\n'));
                            ...
                            if(options.lang === 'ch'){
                                editor.scholatEditorTip("scholatEditorTip-catalogOver", editor);    
                            } else {
                                editor.scholatEditorTip("scholatEditorEnTip-catalogOver", editor);
                            }
                ...
                'foreColor': {
                    ...
                    'type': 'dropPanel',
                    'cssClass': 'icon-ScholatEditor-pencil', 
                    'command': 'foreColor',
                    'style': 'color: #399',
                    'dropPanel': function(){
                        var arr = [],
                ...
                'backgroundColor': {
                    ...
                    'type': 'dropPanel',
                    'cssClass': 'icon-ScholatEditor-brush',  
                    'style': 'color: #399',
                    'command': 'backColor ',
                    'dropPanel': function(){
                        var arr = [],
                ...
                'createLink': {
                    ...
                    'type': 'modal', 
                    'style': 'color: #399',
                    'cssClass': 'icon-ScholatEditor-link',
                    'modal': function (editor) {
                        ...
                        var content = '<p>' +langLink+ '：<input id="' + urlTxtId + '" type="text" style="width:300px;"  placeholder="http://"/></p>' +
                                        ...
                                        '<p><button id="' + btnId + '" type="button" class="ScholatEditor-modal-btn">' +langBtn+ '</button></p>',
                            $link_modal = $(
                        ...
                        $link_modal.find('#' + btnId).click(function(e){
                            ...
                            if(url !== ''){
                                if(!regex.test(url)){
                                    if(options.lang === 'ch'){
                                        editor.scholatEditorTip("scholatEditorTip-errorWebSite", editor);    
                                    } else {
                                        editor.scholatEditorTip("scholatEditorEnTip-errorWebSite", editor);
                                    }
                ...
                'insertExpression': {
                    ...
                    'style': 'color: #399',
                    'command': 'insertImage',
                    'cssClass': 'icon-ScholatEditor-happy',
                ...
                'insertVideo': {
                    ...
                    'type': 'modal',
                    'style': 'color: #399',
                    'cssClass': 'icon-ScholatEditor-play',
                    'modal': function(editor){
                        ...
                        var content = '<p>' +langUrl+ '：<input id="' + txtSrcId + '" type="text" style="width:300px;"  placeholder="http://"/></p>' +
                                        ...
                                        '<p><button id="' + btnId + '" class="ScholatEditor-modal-btn" type="button">' +langBtn+ '</button></p>';
                        var $video_modal = $(
                                $E.htmlTemplates.modalSmall.replace('{content}', content)
                            );
                ...
                'insertTable': {
                    ...
                    'type': 'modal',
                    'style': 'color: #399',
                    'cssClass': 'icon-ScholatEditor-table',
                    'modal': function(editor){
                ...
                'insertImage': {
                    ...
                    'type': 'modal',
                    'style': 'color: #399',
                    'cssClass': 'icon-ScholatEditor-picture',
                    'modal': function (editor) {
                        var //上传组件
                ...
                'insertCode':{
                    ...
                    'type': 'modal',
                    'style': 'color: #399',
                    'cssClass': 'icon-ScholatEditor-terminal',
                    'modal': function(editor){
                        ...
                        var content = '<p>' +langTitle+ '：{selectLangs}</p>' +  // selectLangs 待填充语言列表下拉框
                                        ...
                                        '<button id="' + btnId + '"  type="button" class="ScholatEditor-modal-btn">插入</button>',
                ...
                'insertLocation':{
                    ...
                    'type': 'modal',
                    'style': 'color: #399',
                    'cssClass': 'icon-ScholatEditor-location',
                    'modal': function(editor){
                        ...
                        var content = langCity + '：<input type="text" id="' + txtCityId + '" style="width:60px;"/> ' + 
                                      ...
                                      ' <button class="ScholatEditor-modal-btn" id="' + btnSearchId + '"  type="button">' +langSearch+ '</button>' + 
                                      ' <button class="ScholatEditor-modal-btn" id="' + btnClearId + '"  type="button">' +langClear+ '</button>' +
                                      ' <div id="' + divMapId + '" style="width:100%; height:220px; border:1px solid #ccc; margin:10px 0px;">' +langLoading+ '</div>' +
                                      ' <button class="ScholatEditor-modal-btn" id="' + btnInsertId +'"  type="button">' +langInsert+ '</button>' +
                ...
                'undo': {
                    ...
                    'cssClass': 'icon-ScholatEditor-ccw',
                    'command': 'commonUndo',
                    'callback': function(editor){
                    	//撤销时，会发生光标不准确的问题
                    	//因此，撤销时，让编辑器失去焦点
                ...
                'redo': {
                    ...
                    'type': 'btn',
                    'style': 'color: #399',
                    'cssClass': 'icon-ScholatEditor-cw',
                    'command': 'commonRedo',
                    'callback': function(editor){
                    	//redo时，会发生光标不准确的问题
                    	//因此，redo时，让编辑器失去焦点
                ...
                'viewSourceCode': {
                    ...
                    'type': 'modal',
                    'style': 'color: #399',
                    'cssClass': 'icon-ScholatEditor-code',
                    'modal': function(editor){
                        var txtId = $E.getUniqeId(),
                            btnId = $E.getUniqeId();
                        ...
                        var content = '<div><textarea style="width:100%; height:200px;" id="' + txtId + '"></textarea></div>' +
                                        '<button id="' + btnId + '" class="ScholatEditor-modal-btn"  type="button">' +langUpdate+ '</button>';
                        ...
                        $(function(){
                            //注意，这是一步特殊处理！！！
                            editor.$btnContainer.find('.icon-ScholatEditor-code') //找到<i>
                                                .parent()  //找到 <a> 即 btn
                            .click(function(e){
                ...
                'fullScreen': {
                    ...
                    'type': 'btn',
                    'style': 'color: #399',
                    'cssClass': 'icon-ScholatEditor-enlarge2',
                    'command': 'fullScreen'
                },
                'submit': {
                    'title': langMenus.submit.title,
                    'type': 'btn',
                    'style': 'color: #399',
                    'cssClass': 'icon-ScholatEditor-upload',
                    'command': 'submit',
                    'callback': function(editor){
                        var scholatEditorUserInfoUrl = "";
                        var scholatEditorUserInfo = editor.$txt.html();

                        if( options.lang === 'ch' ){
                            scholatEditorUserInfoUrl = options.url.ch;
                        } else {
                            scholatEditorUserInfoUrl = options.url.en;
                        }
                        //使用ajax保存数据
                        //使用编辑器的后台需要对应的字段有：scholatEditorUserInfo和scholatEditorVersion
                        $.ajax({
                            'type': 'post',
                            'url' : scholatEditorUserInfoUrl,
                            'data': {'scholatEditorUserInfo': scholatEditorUserInfo, 'scholatEditorVersion': options.lang},
                            'success': function(){
                                if(options.lang === 'ch'){
                                    editor.scholatEditorTip("scholatEditorTip-contentSave", editor);    
                                } else {
                                    editor.scholatEditorTip("scholatEditorEnTip-contentSave", editor);
                                }
                                
                            },
                            'error': function(){
                                if(options.lang === 'ch'){
                                    editor.scholatEditorTip("scholatEditorTip-contentFail", editor);    
                                } else {
                                    editor.scholatEditorTip("scholatEditorEnTip-contentFail", editor);
                                }
                ...
                'cancel': {
                    'title': langMenus.cancel.title,
                    'type': 'btn',
                    'style': 'color: #399',
                    'cssClass': 'icon-ScholatEditor-cancel',
                    'command': 'cancel',
                    'callback': function(){
                        window.location.href = options.url.cancel;
                    }
                },
                'reset':{
                    'title': langMenus.reset.title,
                    'type': 'btn',
                    'style': 'color: #399',
                    'cssClass': 'icon-ScholatEditor-eraser',
                    'command': 'reset',
                    'callback': function(editor){
                        var content = $(".ScholatEditor-textarea").html();
                        content = content.replace(/<\/?[^>]*>/g, "");    //去除行html标签
                        content = content.replace(/&nbsp;/g, "");        //去除&nbsp;
    	...
    	'updateMenuStyle': function(){
    	    ...
    	    commandBtns.each(function(){
    	        ...
    	        if($E.commandEnabled(commandName) && document.queryCommandState(commandName)){
    	            $btn.addClass('ScholatEditor-btn-container-btn-selected');
    	        }else{
    	            $btn.removeClass('ScholatEditor-btn-container-btn-selected');
    	        }
    	    });
    	}
    ...
    $.extend($E.fn, {
        ...
        //显示并隐藏scholatEditorTip
        'scholatEditorTip': function(id, editor){
            editor.showScholatHeaderTip(id);
    	...
    	'commandHooks':{
            ...
            'fullScreen': function(commandName, commandValue){
                var $txtContainer = this.$txtContainer,
                    ...
                    enlargeClass = 'icon-ScholatEditor-enlarge2',
                    shrinkClass = 'icon-ScholatEditor-shrink2',
    ...
    $.extend($E.fn, {
    	'command': function(e, commandName, commandValue, callback){
            //如果没有选中，则this.currentRange().collapsed的值为true
            //如果有选中，则this.currentRange().collapsed的值为false
            // console.log( "commandName: " + commandName );
            // console.log( "commandValue: " + commandValue);
            if( !this.currentRange() ){
                console.log('未选中编辑区，无法执行操作');
            }else{
                var commandHook;
                ...
                if($E.commandEnabled(commandName) === true){
                    ...
                    if( commandValue !== '<h1>' ){  
                        //解决的问题有两个：
                        // 1)没有选中区域的时候进行设置目录问题 
                        // 2)选中一个段落的部分，整个段落称为目录
                        // 好处：将所有的设置目录都统一到setHead的callback函数执行
                        if( commandValue === '<p>'){
                            $E.isPassContent = true; 
                        }
    ...
    $.fn.extend({
        ...
        */
        ...
        'ScholatEditor': function(options){
            //检查前台jsp是否使用textarea作为编辑器的编辑区域
            if(this[0].nodeName !== 'TEXTAREA'){
                //只支持textarea
                alert('ScholatEditor提示：请使用textarea扩展富文本框。');
                return;
            }

            //针对一个textarea不能执行两遍 ScholatEditor() 事件
            if(this.data('ScholatEditorFlag')){
                alert('针对一个textarea不能执行两遍ScholatEditor()事件');
                return;
            }else{
                this.data('ScholatEditorFlag', true);
            }
            ...
            //页面刚加载时，初始化提示Modal
            editor.insertScholatEditorModal(editor, options);

            //一分钟自动保存一次
            // var timeout = window.setInterval(function(){
            //     $(".icon-ScholatEditor-upload").click();
            // }, 60000);
